...
const register = async (req, res) => {
  try {
    ...
    if (existingUser) {
      console.log(`Registration attempt with existing email: ${normalizedEmail} at ${new Date().toISOString()}`);
    ...
    if (error.code === 11000) {
      ...
      console.log(`Duplicate key error during registration at ${new Date().toISOString()}`);
      return res.status(409).json({
        message: 'An account with this email already exists. Please try logging in instead.',
        suggestion: 'login'
      });
